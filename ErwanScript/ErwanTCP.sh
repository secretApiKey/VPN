...
MUX_PORTS="${MUX_PORTS:-${MUX_PORT} 80}"
...
NGINX_HTTP_BACKEND_PORT="${NGINX_HTTP_BACKEND_PORT:-8081}"
...
maybe_build_erwanssh_runtime() {
    case "$BUILD_ERWANSSH_RUNTIME" in
        0|false|no)
            echo "Skipping ErwanSSH runtime rebuild."
            return 0
            ;;
        auto)
            if [ -x "${ERWANSSH_DIR}/sbin/sshd" ]; then
                if "${ERWANSSH_DIR}/sbin/sshd" -V >/dev/null 2>&1; then
                    echo "Using bundled ErwanSSH runtime."
                    return 0
                fi
                echo "Bundled ErwanSSH runtime is not compatible; using system sshd."
                rm -f "${ERWANSSH_DIR}/sbin/sshd"
...
ExecStart=
ExecReload=
ExecStartPre=/usr/sbin/sshd -t -f ${SYSTEM_SSH_CONFIG}
ExecStart=/usr/sbin/sshd -D -f ${SYSTEM_SSH_CONFIG}
ExecReload=/usr/sbin/sshd -t -f ${SYSTEM_SSH_CONFIG}
ExecReload=/bin/kill -HUP \$MAINPID
EOF
}
...
install_mode() {
    ...
    echo "ErwanTCP, SSH, and public TCP multiplexers installed on: ${MUX_PORTS}."
    echo "ErwanSSH compatibility listener uses ${ERWANSSH_PORT}; stock admin SSH stays on ${SSH_PORT}."
}

serve_tcp() {
    python3 - "$DOMAIN_FILE" "$MUX_PORTS" "$MUX_TLS_PORT" "$SSH_PORT" "$ERWANSSH_PORT" "$OPENVPN_TCP_PORT" "$INTERNAL_PLAIN_MUX_PORT" "$INTERNAL_TLS_SSL_PORT" "$CUSTOM_HTTP_METHODS" "$CUSTOM_HTTP_METHODS_FILE" "$NGINX_HTTP_BACKEND_PORT" <<'PY'
import asyncio
import struct
import sys
...
public_mux_ports_arg = sys.argv[2]
...
nginx_http_backend_port = int(sys.argv[11])
...
public_mux_ports = parse_port_list(public_mux_ports_arg) or [443]
...
def choose_plain_backend(initial: bytes):
    ...
    if is_tls_client_hello(stripped):
        log_detected("Detected TLS on plain mux, using SSL fallback")
        return "127.0.0.1", tls_ssl_port
    log_detected("Unknown plain mux traffic, forwarding to SSH")
    return "127.0.0.1", erwanssh_port
...
def choose_public_backend(initial: bytes):
    ...
    if stripped.startswith(HTTP_METHODS) or lowered.startswith(HTTP_HEADER_PREFIXES):
        ...
        backend_port = lookup_xray_http_backend(path)
        if backend_port is not None:
            log_detected(f"Detected Xray HTTP path on public mux ({path})")
            return "127.0.0.1", backend_port
        if is_payload_upgrade_request(stripped):
            return "127.0.0.1", 700
        return "127.0.0.1", nginx_http_backend_port
    if is_openvpn_tcp(stripped):
        log_detected(f"Detected OpenVPN (len={len(stripped)})")
        return "127.0.0.1", openvpn_port
    if is_tls_client_hello(stripped):
        ...
        return "127.0.0.1", tls_ssl_port
    log_detected("Unknown public mux traffic, forwarding to SSH")
    return "127.0.0.1", erwanssh_port
...
async def main():
    public_servers = []
    for port in public_mux_ports:
        public_servers.append(await asyncio.start_server(handle_public, host="0.0.0.0", port=port, reuse_address=True))
    plain_server = await asyncio.start_server(handle_plain, host="127.0.0.1", port=plain_mux_port, reuse_address=True)
    await asyncio.gather(
        *(server.serve_forever() for server in public_servers),
