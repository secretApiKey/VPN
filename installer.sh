...
TCP_PORT="${TCP_PORT:-1194}"
UDP_PORT="${UDP_PORT:-110}"
...
HYSTERIA_V1_ALT_PORT="${HYSTERIA_V1_ALT_PORT:-5666}"
HYSTERIA_V1_ALT_RANGE="${HYSTERIA_V1_ALT_RANGE:-20000:50000}"
OVPN_UDP_COMPAT_PORT="${OVPN_UDP_COMPAT_PORT:-1194}"
...
{
  "tcp_port": "${TCP_PORT}",
  "openvpn_tcp_fallback_ports": "1194,443,80",
  "udp_port": "${UDP_PORT}",
  "openvpn_udp_compat_port": "${OVPN_UDP_COMPAT_PORT}",
  "openvpn_udp_fallback_ports": "${UDP_PORT},${OVPN_UDP_COMPAT_PORT}",
...
ensure_real_tls_material() {
    ...
    systemctl restart ErwanTCP >/dev/null 2>&1 || true
...
EOF
    ...
    if [ -n "$OVPN_UDP_COMPAT_PORT" ] && [ "$OVPN_UDP_COMPAT_PORT" != "$UDP_PORT" ]; then
        iptables -t nat -C PREROUTING -p udp --dport "$OVPN_UDP_COMPAT_PORT" -j REDIRECT --to-ports "$UDP_PORT" 2>/dev/null || \
            iptables -t nat -A PREROUTING -p udp --dport "$OVPN_UDP_COMPAT_PORT" -j REDIRECT --to-ports "$UDP_PORT"
    fi
...
udp_port="$UDP_PORT"
ovpn_udp_compat_port="$OVPN_UDP_COMPAT_PORT"
outbound_if="\$(ip route show default 2>/dev/null | awk '/default/ {print \$5; exit}')"
if [ -z "\$outbound_if" ]; then
    outbound_if="\$(ip -o route get 1.1.1.1 2>/dev/null | awk '{for (i=1; i<=NF; i++) if (\$i == "dev") {print \$(i+1); exit}}')"
fi
...
configure_hysteria_udp_forwarding() {
    local primary_port alt_port alt_range

    primary_port="$HYSTERIA_V1_PORT"
    alt_port="$HYSTERIA_V1_ALT_PORT"
    alt_range="$HYSTERIA_V1_ALT_RANGE"

    [ -n "$primary_port" ] || return 0

    if [ -n "$alt_port" ] && [ "$alt_port" != "$primary_port" ]; then
        iptables -t nat -C PREROUTING -p udp --dport "$alt_port" -j REDIRECT --to-ports "$primary_port" 2>/dev/null || \
            iptables -t nat -A PREROUTING -p udp --dport "$alt_port" -j REDIRECT --to-ports "$primary_port"
    fi
    if [ -n "$alt_range" ]; then
        iptables -t nat -C PREROUTING -p udp --dport "$alt_range" -j REDIRECT --to-ports "$primary_port" 2>/dev/null || \
            iptables -t nat -A PREROUTING -p udp --dport "$alt_range" -j REDIRECT --to-ports "$primary_port"
    fi

    cat > /usr/local/bin/erwan-hysteria-udp-forward <<EOF
#!/bin/bash
set -euo pipefail
...
EOF
    sed -i 's/\r$//' /usr/local/bin/erwan-hysteria-udp-forward
    chmod 0755 /usr/local/bin/erwan-hysteria-udp-forward

    cat > /etc/systemd/system/erwan-hysteria-udp-forward.service <<'EOF'
[Unit]
Description=Restore ErwanScript Hysteria alternate UDP entry ports
After=network-online.target udp.service
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/usr/local/bin/erwan-hysteria-udp-forward
RemainAfterExit=yes

[Install]
WantedBy=multi-user.target
EOF
    sed -i 's/\r$//' /etc/systemd/system/erwan-hysteria-udp-forward.service

    systemctl daemon-reload
    systemctl enable --now erwan-hysteria-udp-forward.service >/dev/null 2>&1 || true
}
...
proto udp
remote ${DOMAIN} 110
remote ${DOMAIN} ${OVPN_UDP_COMPAT_PORT}
resolv-retry infinite
connect-retry 3 20
nobind
persist-key
...
systemctl restart ErwanTCP || true
...
systemctl restart nginx ErwanTCP stunnel4 xray udp udp2
        ...
        case "$service" in
            ssh)
                if /usr/sbin/sshd -t -f /etc/ssh/sshd_config; then
                    ...
                    echo "WARNING: stock SSH config failed validation; leaving ssh service unchanged."
                fi
                ;;
            erwanssh)
                if /etc/ErwanSSH/sbin/sshd -t -f /etc/ErwanSSH/etc/sshd_config; then
                    systemctl restart erwanssh >/dev/null || echo "WARNING: failed to restart erwanssh."
                else
                    echo "WARNING: ErwanSSH config failed validation; leaving erwanssh service unchanged."
                fi
                ;;
            *)
...
main() {
    ...
    echo "OpenVPN TCP : ${DOMAIN}:${TCP_PORT} (fallback: 443,80)"
    echo "OpenVPN UDP : ${DOMAIN}:${UDP_PORT} (fallback: UDP ${OVPN_UDP_COMPAT_PORT})"
